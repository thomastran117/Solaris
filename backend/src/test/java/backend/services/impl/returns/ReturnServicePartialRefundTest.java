...
class ReturnServicePartialRefundTest {
    ...
    void issuePartialRefund_createsReturnAndFiresStripeRefund() throws Exception {
        ...
        PaymentService.RefundResult refundResult =
                new PaymentService.RefundResult("re_test123", 500L, "usd", "pending", "pi_test");
        ...
        when(returnRepository.save(any())).thenAnswer(inv -> {
            ...
            ret.setId(TestIds.uuid(99));
            return ret;
        });

        ReturnResponse resp = service.issuePartialRefund(TestIds.uuid(10), 500L, "Damaged in transit", TestIds.uuid(2));

        assertEquals(TestIds.uuid(99), resp.id());
    ...
    void issuePartialRefund_throwsWhenAmountZero() {
        ...
        when(orderRepository.findById(TestIds.uuid(10))).thenReturn(Optional.of(makeOrder(TestIds.uuid(10), makeUser(TestIds.uuid(1)))));
        assertThrows(BadRequestException.class,
                () -> service.issuePartialRefund(TestIds.uuid(10), 0L, null, TestIds.uuid(2)));
    }
    ...
    void issuePartialRefund_throwsWhenOrderNotFound() {
        ...
        assertThrows(ResourceNotFoundException.class,
                () -> service.issuePartialRefund(TestIds.uuid(99), 500L, null, TestIds.uuid(2)));
    }
    ...
    void issuePartialRefund_throwsWhenActorIsNotStaff() {
        ...
        assertThrows(ForbiddenException.class,
                () -> service.issuePartialRefund(TestIds.uuid(10), 500L, null, TestIds.uuid(2)));
    }
