...
import { Loader2, Truck, Package, CheckCircle, MapPin } from "lucide-react";
...
export default function FulfillmentActionBar({ order, companyId, onRefresh }: Props) {
  ...
  const isLoading = pack.isPending || ship.isPending || pickupReady.isPending || deliver.isPending;
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur p-4 flex flex-col gap-3">
      ...
      <div className="flex flex-wrap gap-3">
        {isLoading ? <Loader2 className="h-5 w-5 text-white/40 animate-spin" /> : buttons}
      </div>
