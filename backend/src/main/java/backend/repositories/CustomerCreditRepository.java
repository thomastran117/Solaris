...
public interface CustomerCreditRepository extends JpaRepository<CustomerCredit, UUID> {
    ...
    List<CustomerCredit> findAllByUserIdOrderByCreatedAtDesc(UUID userId);

    Optional<CustomerCredit> findFirstBySourceOrderIssueId(UUID sourceOrderIssueId);
    ...
    long sumBalanceByUserId(@Param("userId") UUID userId, @Param("now") Instant now);
    ...
    List<CustomerCredit> findAllByUserIdForUpdate(@Param("userId") UUID userId);
    ...
    int claimForReversal(@Param("id") UUID id);
